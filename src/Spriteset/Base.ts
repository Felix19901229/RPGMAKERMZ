...
export class Spriteset_Base extends Sprite {
    ...
    public updateBaseFilters() {
        ...
        filter.setColorTone(window.$gameScreen.tone());
    };

    public updateOverallFilters() {
        const filter = this._overallColorFilter;
        filter.setBlendColor(window.$gameScreen.flashColor());
        filter.setBrightness(window.$gameScreen.brightness());
    };
    ...
    public processAnimationRequests() {
        for (; ;) {
            const request = window.$gameTemp.retrieveAnimation();
            if (request) {
    ...
    public createAnimation(request) {
        const animation = window.$dataAnimations[request.animationId];
    ...
    public createAnimationSprite(targets, animation, mirror, delay) {
