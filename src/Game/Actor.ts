import { TextManager ,SoundManager,DataManager,BattleManager} from "../Manager/index.js";
...
export class Game_Actor extends Game_Battler {
    ...
    public displayLevelUp(newSkills) {
        ...
        for (const skill of newSkills) {
            window.$gameMessage.add(TextManager.obtainSkill.format(skill.name));
        }
    }
    ...
    public showAddedStates() {
        for (const state of this.result().addedStateObjects()) {
            if (state.message1) {
                window.$gameMessage.add(state.message1.format(this._name));
            }
        }
    }
    ...
    public showRemovedStates() {
        for (const state of this.result().removedStateObjects()) {
            if (state.message4) {
                window.$gameMessage.add(state.message4.format(this._name));
            }
        }
    }
    ...
    public lastMenuSkill() {
        return this._lastMenuSkill.object() as Skill;
    }
    ...
    public lastBattleSkill() {
        return this._lastBattleSkill.object() as Skill;
    }
