...
export class Game_Map {
    ...
    setupEvents() {
        this._events = [];
        this._commonEvents = [];
        for (const event of window.$dataMap.events.filter(event => !!event)) {
    ...
    autorunCommonEvents() {
        return window.$dataCommonEvents.filter(commonEvent => commonEvent && commonEvent.trigger === 1);
    }
    ...
    setupAutorunCommonEvent() {
        for (const commonEvent of this.autorunCommonEvents()) {
            if (window.$gameSwitches.value(commonEvent.switchId)) {
